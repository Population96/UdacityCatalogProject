...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/v2.4/oauth/' \
          ...
          '&client_secret=%s&fb_exchange_token=%s' % (app_id, app_secret,
                                                      access_token)
    h = httplib2.Http()
    ...
    url = 'https://graph.facebook.com/v2.4/me?fields=name,id,email' \
...
# EDIT A MENU ITEM
@app.route('/catalog/<int:category_id>/item/<int:item_id>/edit/', methods=[
    'GET', 'POST'])
...
# DELETE A MENU ITEM
@app.route('/catalog/<int:category_id>/item/<int:item_id>/delete/', methods=[
    'GET', 'POST'])
